...
function AssignmentEditor({period,plan,subject,campuses,onSaved,onClose,existing}){
  ...
  const save=async()=>{setBusy(true);setError('');try{
    ...
    await saveAcademic(prefix+'/asignar',{...form,docente_id:Number(form.docente_id),sede_id:form.sede_id?Number(form.sede_id):null,asignacion_id:existing?.id,previous});onSaved();
...
export default function CurriculumPlanning({cuatrimestre,puedeEditar=false,onCatalog,onSaved}){
  ...
  const saveOffer=async()=>{setBusy(true);setError('');try{await saveAcademic('/api/planificacion/'+cuatrimestre+'/planes/'+selected+'/oferta',{revision:data.oferta_revision,materia_ids:selectedSubjects},'PUT');setAttempt(a=>a+1);}catch(e){setError(e.message);}finally{setBusy(false);}};
  ...
  return <div className="academic-catalog">
    ...
    {data&&<>
      ...
      {editor&&<AssignmentEditor key={editor.subject.id+':'+(editor.existing?.id||'new')} period={cuatrimestre} plan={selected} subject={editor.subject} existing={editor.existing} campuses={campuses} onSaved={()=>{setEditor(null);setAttempt(a=>a+1);onSaved?.();}} onClose={()=>setEditor(null)}/>}
